...
async def test_active_transport_raises_when_disconnected_mqtt_reported_offline() -> None:
    """mqtt_reported_offline=True and disconnected MQTT with no BLE has no usable transport."""
    mqtt = make_transport(TransportType.CLOUD_ALIYUN, connected=False)
    handle = make_handle()
    await handle.add_transport(mqtt)

    handle.update_availability(TransportType.CLOUD_ALIYUN, TransportAvailability.DISCONNECTED, mqtt_reported_offline=True)
